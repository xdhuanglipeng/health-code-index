...
function setDynamicTime(selector, start = 0, end = 19) {
	...
	}), updateTime(), time_updater_init || (time_updater_init = !0, window.setInterval(updateTime, time_interval))
}
...
function setStaticTime(selector, start = 0, end = 19, traceback_hours = 0, traceback_range = 0, filter = x => x) {
	for (const element of document.querySelectorAll(selector)) {
		var hours = element.attributes["data-traceback-hours"] ? .value ? parseFloat(element.attributes["data-traceback-hours"] ? .value) + (Math.random() - .5) * parseFloat(element.attributes["data-traceback-range"] ? .value || 0) : traceback_hours + (Math.random() - .5) * traceback_range,
			hours = new Date((new Date)
				.getTime() + 288e5 - 3600 * hours * 1e3)
			.toISOString()
			.replace("T", " ");
...
function addStorageField(id, selector, name, placeholder, filter = x => x, callback = () => {}) {
	var elements = document.querySelectorAll(selector),
		...
		init_value = (id in fields ? fields[id].push(selector) : fields[id] = [selector], localStorage.getItem(id) || placeholder);
	for (const element of elements) element.innerText = filter(init_value), element.addEventListener("click", () => {
		var res = window.prompt("修改" + name + "：", localStorage.getItem(id) || placeholder);
		"" == res || null == res ? localStorage.removeItem(id) : localStorage.setItem(id, res), callback(res || placeholder);
